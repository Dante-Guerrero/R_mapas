...
```{r}
mx <- st_read("insumos/geometrias/demarcacion/pais/geo.shp")
...
## DEMARCACIÓN POR DEPARTAMENTO

```{r}

mx <- st_read("insumos/geometrias/demarcacion/departamentos/geo.shp")
...
## DEMARCACIÓN POR PROVINCIA

```{r}

mx <- st_read("insumos/geometrias/demarcacion/provincias/geo.shp")
...
## DEMARCACIÓN POR DISTRITO

```{r}

mx <- st_read("insumos/geometrias/demarcacion/distritos/geo.shp")
...
```{r}
mx <- st_read("insumos/geometrias/volcanes/geo.shp")
mx_data <- select(
  as.data.frame(mx), 
  NOMBRE,
  CODIGO,
...
```{r}
mx <- st_read("insumos/geometrias/lagunas_glaciares/geo.shp")
...
```{r}
mx <- st_read("insumos/geometrias/lago_titicaca/geo.shp")
...
for(i in departamentos){
    ubicacion_de_archivo <- gsub(" ", "", paste('insumos/geometrias/', carpeta, '/', i, '/geo.shp'))
...
mx <- st_read("insumos/geometrias/red_vial_nacional/geo_Eje.shp")
mx_data <- select(
  as.data.frame(mx),
  RUTA = cCodRuta,
  LONGITUD = dLongitud,
...
mx <- st_read("insumos/geometrias/red_vial_nacional/geo_Pt.shp")
mx_data <- select(
  as.data.frame(mx),
  COD_RUTA,
  NOMB_POB,
...
mx <- st_read("insumos/geometrias/red_vial_departamental/geo_Eje.shp")
mx_data <- select(
  as.data.frame(mx),
  COD_DS11,
  COD_DS12,
...
mx <- st_read("insumos/geometrias/red_vial_departamental/geo_Pt.shp")
mx_data <- select(
  as.data.frame(mx),
  COD_RUTA,
  POSICION,
...
mx <- st_read("insumos/geometrias/red_vial_vecinal/geo_Eje.shp")
mx_data <- select(
  as.data.frame(mx),
  COD_DS11,
  COD_DS12,
...
mx <- st_read("insumos/geometrias/red_vial_vecinal/geo_Pt.shp")
mx_data <- select(
  as.data.frame(mx),
  COD_RUTA,
  POSICION,
...
```{r}
mx <- st_read("insumos/geometrias/red_vial_trochas/geo.shp")
mx_data <- select(
  as.data.frame(mx),
  ESTADO = estado,
  DESCRIPCION = descrip,
